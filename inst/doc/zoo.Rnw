...
<<rapply>>=
rapply(Z, 5, sd)
rapply(Z, 5, sd, na.pad = TRUE, align = "left")
@
...
this can be exploited for coercion to other time classes such as \code{"Date"},
\code{"POSIXct"} or \code{"POSIXlt"} which by default associate the first day
...
First, we transform the index of the regular series \code{zr2}
to \code{"yearmon"} to improve printing.

<<yearmon2>>=
index(zr2) <- yearmon(index(zr2))
zr2
@
...
<<yearmon4>>=
index(zr2) <- as.POSIXct(index(zr2))
as.irts(zr2)
@
...
top of it. Furthremore, it bridges the gap between irregular and regular series,
