...
if(online) {
  sunw <- get.hist.quote(instrument = "SUNW", start = "2004-01-01", end = "2004-12-31", origin = "1970-01-01")
  sunw <- na.omit(as.zoo(sunw))
  attr(sunw, "na.action") <- NULL
  time(sunw) <- as.Date(time(sunw))

  sunw2 <- get.hist.quote(instrument = "SUNW", start = "2004-01-01", end = "2004-12-31",
    origin = as.Date(0), compression = "m", quote = "Close")
  sunw2 <- na.omit(as.zoo(sunw2[, 1]))
  attr(sunw2, "na.action") <- NULL
  time(sunw2) <- as.Date(time(sunw2))
} else {
  load("sunw.rda")
}
@
...
dates:
<<transform to zoo with Date index, eval=FALSE>>=
sunw <- na.omit(as.zoo(sunw))
attr(sunw, "na.action") <- NULL
time(sunw) <- as.Date(time(sunw))
@
